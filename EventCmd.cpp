...
void EventHandler::connectServer() {
...
void EventHandler::cmdPass() {
	...
	// 접속중인지 확인하는게 첫번째
	// 접속중이 아니면 pass 변경 이후 변경된 pass 와 서버 pass 비교해서 맞으면 접속 시그널 날려주기
}
void EventHandler::cmdNick() {
	_fdTo.insert(_fdFrom);
	// ":nick NICK :newnick\r\n"
	std::map<int, FileDescriptor>::iterator fditer = _fdList.find(_fdFrom);
	if (fditer == _fdList.end())
		return ;
	std::vector<std::pair<int, std::string> >::iterator it = _parse.begin();
	while (it != _parse.end()) {
		if (it->first == MIDDLE)
...
void EventHandler::cmdJoin() {
	if (!_fdList.find(_fdFrom)->second.getWelcome())
		return;
	std::vector<std::pair<int, std::string> >::iterator it = _parse.begin();
	while (it != _parse.end()) {
		if (it->first == MIDDLE)
	...
	for(size_t i = 0; i < channel.size(); i++) {
		if (channel[i] == ',' && channel[start] != '#') {
			...
			if (_channelList.count(channelName) && _channelList.find(channelName)->second.isInChannel(_fdFrom))
 				continue;
			...
			int operFd = _channelList.find(channelName)->second.getOper();
			std::string nick;
			if (operFd != -1) {
...
void	EventHandler::cmdKick()
{
	...
	if (channel[0] == '#') {
		...
		std::string operNick = _fdList.find(_channelList.find(channelName)->second.getOper())->second.getNick();
		if (operNick != nick) {
			_fdTo.insert(_fdFrom);
